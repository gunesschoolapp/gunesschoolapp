...
export default function IncomeLedgerTab() {
  ...
  const months = [...new Set(payments.map(p => p.payment_date?.slice(0, 7)).filter(Boolean))].sort().reverse();

  const filtered = payments.filter(p => {
    const student = getStudent(p.student_id);
    const matchSearch = !search || (student?.full_name || '').toLowerCase().includes(search.toLowerCase());
                ...
                filtered.map((p, i) => {
                  ...
                  return (
                    <tr key={p.id} className={`border-b border-border last:border-0 hover:bg-muted/30 transition-colors ${i % 2 === 0 ? '' : 'bg-muted/10'}`}>
                      ...
                      <td className="px-4 py-3 text-muted-foreground">{course?.name || '-'}</td>
                      <td className="px-4 py-3 text-muted-foreground">
                        {p.total_installments > 1 ? `${p.installment_number}/${p.total_installments}` : '-'}
                      </td>
                      <td className="px-4 py-3 text-muted-foreground">{methodLabel[p.payment_method] || '-'}</td>
