...
export default function InvoicesTab() {
  ...
  const filtered = invoices.filter(inv => {
    const matchSearch = !search || (inv.invoice_number || '').toLowerCase().includes(search.toLowerCase()) || (inv.student_name || '').toLowerCase().includes(search.toLowerCase());
    ...
    <div className="space-y-4">
                  ...
                  <tr key={inv.id} className={`border-b border-border last:border-0 hover:bg-muted/30 ${i % 2 === 0 ? '' : 'bg-muted/10'}`}>
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => setPreviewInvoice(inv)} className="text-muted-foreground hover:text-primary transition-colors p-1">
      ...
      <Dialog open={!!previewInvoice} onOpenChange={() => setPreviewInvoice(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {previewInvoice && activeTemplate && (
            <InvoicePreview invoice={previewInvoice} template={activeTemplate} />
          )}
